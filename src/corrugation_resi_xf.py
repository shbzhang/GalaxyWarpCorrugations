### obsolete, replaced by corrugation_resi.py
### plot Az-(Z-warp model) of MCs with Xfactor correction in different Rgal
...
if __name__ == '__main__':
	...
	for i in range(len(theta_sep)-2):
		...
		if i == 10:
			...
			ax[i].set_ylabel('$\mathbf{\Delta}$Z (kpc)', fontsize=13, fontweight='bold')
		if i>=10:
	...
	plt.savefig('fig/r_z_corrugation_resi_%icomp_xf.png' % comp,format='png',bbox_inches='tight', dpi=400)
	plt.show()

'''
##substract the warp model
import numpy as np
import pylab as pl
import scipy.interpolate as sp_interp
from scipy.stats import norm
...
if __name__ == '__main__':
	...
	osc_data = np.loadtxt('osc2_para_xf.txt',comments='#')
	per_data = np.loadtxt('per_para_xf.txt',comments='#')
	residual = np.loadtxt('residual_xf_1com.razm')
	col,col1,col_hi,col_cepheid,col_co,col_cen,col_err = ['coral','none','teal','blue','crimson','black','#36454F']
...
#	fig1.tight_layout()
	...
	pl.text(8.5,0.8,'[%s,%s]' %(thea[0],thea[1]),fontsize=11,fontweight='bold')
	pl.xlim([8,21.5])
	pl.ylim([-0.9,0.9])
...
#	pl.plot(rg,zw5,'--',color='green')
	pl.plot([8,21.5],[0,0],ls='dotted',color='red',lw=1)
	pl.xticks([8,10,12,14,16,18,20],['8','10','12','14','16','18','20'],fontsize=11,fontweight='bold')
	pl.yticks([-0.8,-0.4,0,0.4,0.8],['','','','',''],fontsize=11,fontweight='bold')
	#125_115
	thea=[124,112]
	rg,zw1,zw2,zw3,zw5,zw6,w=cal_warpc((thea[0]+thea[1])/2.)
...
#	pl.plot(rg,zw5,'--',color='green')
	...
	fig1.savefig('/home/yansun/HDwmisp/survey/reduced/r_z_corrugation_residual_xf.png',format='png',bbox_inches='tight')
