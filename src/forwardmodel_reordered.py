...
def weighted_binned_statistic(x, z, w, bins):
	...
	std_z = np.zeros(len(bins) - 1)
	for i in range(len(bins) - 1):
		mask = (x >= bins[i]) & (x < bins[i + 1])
		if np.sum(w[mask]) > 0:
			variance = np.average((z[mask] - mean_z[i]) ** 2, weights=w[mask])
			std_z[i] = np.sqrt(variance)
		else:
			std_z[i] = np.nan
	return 0.5 * (bins[:-1] + bins[1:]), mean_z, std_z


def load_real_data():
	#read clouds
	columns = [
		'l_cen', 'b_cen', 'v_cen', 'lwidth', 'distance', 'err_dist', 'size_pc', 'mass',
		'complete', 'surface_density', 'viral_para', 'gal_ridus', 'xx', 'yy', 'zz',
		'index', 'l_rms', 'b_rms', 'v_rms', 'beta', 'T_int', 'area', 'T_peak', 'tex',
		'nh2', 'pur', 'fillingfactor', 'l_peak', 'b_peak', 'v_peak'
	]
	files = ['out_para.txt', 'per_para.txt', 'osc2_para.txt']
	dfs = []
	for f in files:
		if os.path.exists(f):
			dfs.append(pd.read_csv(f, sep=r'\s+', names=columns))
			print(f"Loaded {f}")
		else:
			raise ValueError("Warning: %s not found." % f)

	df_real = pd.concat(dfs, ignore_index=True)
	df_real = df_real[(df_real['distance'] > 0) & (df_real['mass'] > 0)].copy()
	return df_real


def a5_vlsr_from_distance(l_deg, b_rad, d_kpc, unit=A5):
	#Compute standard V_lsr from (l, b, d) using the same A5 kinematic model
	...
	# Old IAU solar motion used internally in Distance.py
	Uo_IAU = 10.27
	Vo_IAU = 15.32
	Wo_IAU = 7.74
	...
	# Projected heliocentric distance in the Galactic plane
	d_proj = d_kpc * cos_b

	# Galactocentric radius
	Rs = np.sqrt(Ro**2 + d_proj**2 - 2.0 * Ro * d_proj * cos_l)

	# beta angle (same geometry as Distance.calc_Dk)
	beta = np.arctan2(d_proj * sin_l, Ro - d_proj * cos_l)
	...
	# Projected planar velocity term consistent with Distance.kinematic_distance()
	V_proj = (Theta * Ro / Rs - To) * sin_l

	# Undo the projection used in calc_Dk()
	v_fixed = V_proj / cos_b

	# Re-apply the mean source peculiar motions
	v_newlsr = (
		v_fixed
		+ (Vs * np.sin(gamma) - Us * np.cos(gamma)) * cos_b
		+ Ws * sin_b
	)

	# Convert from "new LSR" back to heliocentric
	v_helio = v_newlsr - (Vo * sin_l + Uo * cos_l) * cos_b - Wo * sin_b

	# Convert heliocentric to standard LSR
...
def plot_direct_streaming_faceon(x_true, y_true, v_systematic, pitch_angle_deg=12.0, v_sys_amp=6.0,\
	save_prefix='Empirical_DirectLOS_Paper'):
    # face-on map of injected streaming
...
def _solve_kinematic_with_A5(l_true_deg, l_true_rad, V_obs, b_rad):
    # solve kinematic distance using A5, return R, phi, Z, d
	R_sun = A5['Ro']
...
def _build_mock(df_real, pitch_angle_deg=12.0, v_sys_amp=6.0, v_rand_amp=3.0,
    ...
    # build mock catalog with radial warp or flat disk
	...
	# assume amp of 200 pc, std is 0.7*200~140pc
	# intrinsic sigma ~ sqrt(thickness**2 - amp_std**2) ~ 60 pc
...
def draw_figures(\
	df_real, baseline_kwargs=None,\
	stress_kwargs=None,\
	radial_sector_phi=(29.0, 38.0),\
	radial_bins=np.arange(9.0, 16.0, 0.6),\
	save_prefix='Empirical_DirectLOS_Paper'):
    # main plotting function, makes 4 subplots and face-on maps
...
if __name__ == '__main__':
	...
	print(df_catalog)
    # print("distance range:", df_catalog['distance'].min(), df_catalog['distance'].max())
	diagnostics = draw_figures(df_catalog)
	print(diagnostics)
