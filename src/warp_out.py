...
if __name__ == '__main__':
	...
	if 1:
		...
		powerAxes.text(bestFreq+0.11, peak+0.03, '%.1f kpc' % (1/bestFreq), ha='left', va='center', font = dict(size=20))
...
if 0:
	...
	length = f(az)
	### plot clouds
	ax[0].scatter(length, zz_res, s=scatter_size*5, **arm_kws_mc)

	### plot binned average
	lencen, lenrms, zcen_res, zrms_res= cal_zcen_zrms(length, zz_res, weights=mass, binsize=-1.5, bin0=0)
