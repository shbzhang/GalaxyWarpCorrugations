...
if __name__ == '__main__':
	...
	ax[0].scatter(az, zz, s=scatter_size, c=col_mc, edgecolor='none', alpha=0.4, label='OSC')
	...
	ax[0].plot(azcen[idx], zcen[idx], '--', c=darker_hex(col_mc), lw=2, alpha=0.8)
	...
	ax[0].minorticks_on()
	ax[0].set_xlim(-32, 162)
	ax[0].set_ylim(-0.7, 1.5)

	# upper tick
	import scipy.interpolate as sp_interp
	phiAxis = np.linspace(-32, 162, 200)
	lAxis = []
	for phi in phiAxis:
		lAxis.append(cal_arm_length(-32, phi, p=best_osc))
	...
	upper.set_xlim(-32, 162)

	### plot cloud
	#zz_res = zz - function_warp((rr, az))
	zz_res = zz - function_warp( (function_arm(az, best_osc), az) )
	ax[1].scatter(az, zz_res, s=scatter_size, c=col_mc, edgecolor='none', alpha=0.4, label='Residual')
	...
	ax[1].plot(azcen[idx], zcen_res[idx], '--', c=darker_hex(col_mc), lw=2, alpha=0.8)

	### plot H line
	ax[1].plot([-30, 150], [0, 0], sty_co1, color=col_co, lw=2)
