...
if __name__ == '__main__':
	...
	### calculate arm
	PHI = np.linspace(az.min(), az.max(), 300)
	...
	ax[0].scatter(az, zz, s=scatter_size, c=col_mc, edgecolor='none', alpha=0.3, label='Outer')
	...
	ax[0].minorticks_on()
	ax[0].set_xlim(-30, 160)
	ax[0].set_ylim(-0.5, 0.75)


	# upper tick
	import scipy.interpolate as sp_interp
	phiAxis = np.linspace(-30, 160, 200)
	lAxis = []
	for phi in phiAxis:
		lAxis.append(cal_arm_length(-30, phi, p=best_out))
